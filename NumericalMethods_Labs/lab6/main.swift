...
func dividingRootEquation(of function: (Float) -> Float, in section: (a: Float, b: Float), with inaccuracy: Float) -> Float {
...
func tangentRootEquation(of function: (Float) -> Float, and derivative: (first: (Float) -> Float, second: (Float) -> Float),
                         in section: (a: Float, b: Float), with inaccuracy: Float) -> Float
{
...
sections.forEach { section in
    let root1 = dividingRootEquation(of: f, in: section, with: eps)
    let root2 = tangentRootEquation(of: f, and: (f_x, f_xx), in: section, with: eps)
    print(root1, root2, abs(root1 - root2))
}
